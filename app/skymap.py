...
def generate_skymap(
    ...
    for seg in const_lines:
        ...
        if a in hip_xy and b in hip_xy:
            ...
            lines_svg.append(
                ...
                f'data-constellation="{html.escape(name.replace(" ", "-"), quote=True)}" '
    ...
    for s in stars:
        ...
        if members:
            tag = html.escape(" ".join(m.replace(" ", "-") for m in sorted(members)), quote=True)  # ponytail: input trusted today; guards future data edits
            stars_svg.append(
